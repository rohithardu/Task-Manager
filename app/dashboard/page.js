...
const dashboard = () => {
  ...
  return (<>
    <div className='flex items-center justify-between px-5 py-4 bg-green-400'>
    ...
    <div className='flex gap-2 bg-gray-100 min-h-[90vh]'>
      <div className="left w-[20vw] flex flex-col gap-4 border-r-2 border-white py-4">
        <button onClick={() => setcreate(true)} className="px-8 py-2 bg-green-400 w-2/3 mx-auto rounded-full cursor-pointer font-semibold">Create Task</button>
        <div className='w-full  text-lg border-t-2 border-gray-500'>
          <h2 onClick={() =>allTaskClick()} className='font-semibold text-center hover:cursor-pointer hover:bg-white py-2'>All tasks</h2>

          <div onClick={() => setcreate(false)} className='w-full text-lg border-y border-gray-500'>
            <h3 className='p-2'>Categories</h3>
            <div>
              {allCategoryArray && allCategoryArray.map((item, index) => {
                const category = item.split(" ")[0]
                console.log(category)
                return <div key={index} onClick={(e)=> {e.stopPropagation()
                   searchByCategory(category)}} className='py-1 border-t border-gray-300  px-4 cursor-pointer hover:cursor-pointer hover:bg-white capitalize'>{item}</div>
              })}
            </div>
          </div>
        </div>
      </div>
      <div className="right py-8 w-[80vw]">
        <div className={`flex flex-col gap-5 items-center ${create ? "" : "hidden"}`}>
          <label htmlFor="category">Category</label>
          <input type="text" onChange={handleChange} onKeyDown={handleKeyDown} value={form.category} name='category' id='category' className='bg-white w-1/3 px-4 py-1 rounded-lg h-10 outline' placeholder='Category (Work,Personal etc.)' />
          <label htmlFor="input">Input</label>
          <input type="text" onChange={handleChange} onKeyDown={handleKeyDown} value={form.input} name='input' id='input' className='bg-white w-1/3 px-4 py-1 rounded-lg h-10 outline' placeholder='Create your task' />
          <label htmlFor="tags">Tags</label>
          <input type="text" onChange={handleChange} onKeyDown={handleKeyDown} value={form.tags} name='tags' id='tags' className='bg-white w-1/3 px-4 py-1 rounded-lg h-10 outline' placeholder='Tags (optional)' />
          <label htmlFor="deadline">Deadline</label>
          <input type="date" onChange={handleChange} onKeyDown={handleKeyDown} value={form.deadline} name='deadline' id='deadline' className='bg-white w-1/3 px-4 py-1 rounded-lg h-10 outline' placeholder='Deadline (optional)' />
        ...
        <div className={`mx-auto flex flex-col gap-4 justify-start ${create ? "hidden" : ""}`}>
          <div className='flex items-center justify-between px-4'>
            <div className='flex gap-4 items-center'>
              <span onClick={() => setpending(true)} className={`${pending?"bg-gray-400":"bg-white"} px-3 py-2 rounded-full cursor-pointer`}>Pending</span>
            ...
            <div className='relative'>
              <input type="text" placeholder='Search' name='search' id='search' value={search} onChange={find} className='bg-white px-3 py-1 rounded-full w-62.5 outline' />
              <img src="search.svg" alt="search" width={15} height={15} className='absolute right-3 top-2' />
            </div>
          </div>
          <div className="tags">
            <div className={`pendingTags ${pending ? "" : "hidden"}`}>
              {pendingShowTags && pendingShowTags.map((item, index) => {
                const tag = item.split(" ")[0]
                // console.log(tag)
                return <span key={index} onClick={()=>searchByTag(tag)} className='bg-white cursor-pointer rounded-full px-3 py-1 mx-1'>#{item}</span>
              })}
            </div>
            <div className={`completedTags ${pending ? "hidden" : ""}`}>
            ...
            <div className={`${pending ? "" : "hidden"} flex flex-col gap-4 justify-center`}>
              ...
              return <div key={item._id} className={`flex items-center justify-between gap-3 text-lg rounded-lg px-3 bg-white ${item.completed ? "hidden" : ""}`}>
                <div className='flex items-center gap-2'>
                  ...
                  <span className='break-all'>{item && item.task}</span>

                </div>
                <div className='flex gap-3 items-center'>
                  <span className='text-sm'>{item && item.deadline}</span>
