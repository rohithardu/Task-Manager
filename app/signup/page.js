"use client"
import React from 'react'
import Link from 'next/link'
import { useState } from 'react'
import { useRouter } from 'next/navigation'

const signup = () => {
  const router = useRouter()
  const [form, setform] = useState({name:"", number:"", password:""})

  const handleChange = (e) => {
    setform({...form, [e.target.name]: e.target.value}) 
  }

  const register = async() => {
    if(form.name ==="" || form.number ==="" || form.password === ""){
      alert("Please fill all fields")
      return
    }
    if(form.number.length !== 10 || (!form.number.startsWith(9) && !form.number.startsWith(8) && !form.number.startsWith(6) &&!form.number.startsWith(7) )){
      alert("Please enter a valid number")
      return
    }
    if(form.password.length<8|| !/[A-Z]/.test(form.password)||!/[a-z]/.test(form.password)||!/[0-9]/.test(form.password)){
      alert("Please enter a valid password")
      return
    }
    const myHeaders = new Headers();
myHeaders.append("Content-Type", "application/json");
const split = form.name.split("").slice(1).join("")
// console.log(split)
const newName = form.name.charAt(0).toUpperCase() + split
// console.log(newName)

const raw = JSON.stringify({
  "name": newName,
  "number": form.number,
  "password": form.password
});

const requestOptions = {
  method: "POST",
  headers: myHeaders,
  body: raw,
  redirect: "follow"
};

const res = await fetch("api/auth/signup", requestOptions)
const result = await res.json()
if(!res.ok){
  return
}
router.push("/dashboard")
  setform({name:"", number:"", password:""})
  }
  
  

  
  return (<>
  <div className='bg-gray-100 min-h-screen flex justify-center items-center'>
    <div className='bg-white h-[70vh] w-[90vw] md:w-[50vw] shadow-lg flex flex-col justify-center items-center gap-5'>
        <input type="text" onChange={handleChange} value={form.name} name='name' id='name' className='bg-gray-100 px-4 py-1 rounded-lg w-[80%] sm:w-2/3 md:w-1/2 h-10 outline' placeholder='Enter your name' />
        <input type="number" onChange={handleChange} value={form.number} name='number' id='number' className='bg-gray-100 px-4 py-1 rounded-lg w-[80%] sm:w-2/3 md:w-1/2 h-10 outline' placeholder='Enter your number' />
        <input type="text" onChange={handleChange} value={form.password} name='password' id='password' className='bg-gray-100 px-4 py-1 rounded-lg w-[80%] sm:w-2/3 md:w-1/2 h-10 outline' placeholder='Enter your password' />
        <button onClick={register} className="px-4 md:px-8 py-2 bg-green-400 rounded-full cursor-pointer font-semibold">Signup</button>
        <Link href={"/login"}>
        <div className='text-blue-400 cursor-pointer my-5'>Existing User? Login</div>
        </Link>
    </div>
  </div>
  </>
  )
}

export default signup
